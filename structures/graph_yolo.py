...
class FrameGraph:
    ...
    def create_graph(self, graph_path):
        ...
        for triple_dict in triples:
            sub = triple_dict["type"]
            pred = triple_dict["type"]
            obj = triple_dict["type"]
            ...
            obj_obj = SceneObject(sub["id"], obj["xmin"], obj["ymin"], obj["xmax"], obj["ymax"], obj["id"], predicate)
                    ...
                    for node1, node2, data in edges:
                        if data["relation"] in static_relationships:
                            if framegraph.g.nodes[node1][
                                "content"].cuid in self.g.nodes():  # and self.leaf_node_id[node1.cuid] in self.g.nodes():
                                if node2 in Bestmatch.keys():
                                    if node2.name == self.g.nodes[Bestmatch[node2]]["content"].name:
                                        cui = self.g.nodes[Bestmatch[f]]["content"].get_cuid()
                                        framegraph.g.nodes[f]["content"].cuid = cui
                                        pos = self.g.nodes[Bestmatch[f]]["content"].get_pos()
                                        framegraph.g.nodes[f]["content"].pos = pos
                                        relationship = self.g.nodes[Bestmatch[f]]["content"].get_relationship()
