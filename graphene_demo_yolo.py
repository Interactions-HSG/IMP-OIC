...
from structures.graph_yolo import *
...
class Graphene:
    ...
    def run_online(self, graph2text):
        ...
        while True:
            ...
            generate_scene_graph("YOLOv8", os.path.join(CAM_PATH, img_name),
    ...
    def classify_images(self, image_path):
        ...
        for image in tqdm.tqdm(images):
            generate_scene_graph("YOLOv8", image_path + "/" + image,
...
def generate_scene_graph(yolo_path, img_path, graph_path):
    """
    calls Yolo to create scene graph from image and saves json output file in graph path
    """
    try:

        subprocess.check_output([f'python',
                                 f"{yolo_path}/predict.py",
                                 "--img_path", f"{img_path}",
